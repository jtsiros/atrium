...
`setup` does the build, and installs Rust for you if it is missing.
...
| Switches, fans, humidifiers, remotes | On/off, and fan speed |
...
`←`/`→` change room, `enter` switches a device on or off, `e` opens its
controls, `r` refreshes, `s` opens settings, `tab` moves to the next bar panel,
and `esc` closes.
...
address, your room preferences, and what you have pinned.

## Security

Your access token is stored in your system keyring.

Certificates are checked, and proxy settings in your environment are ignored. An
address with no `https://` is treated as secure; a plain `http://` address works
but warns you first, because it sends your token unencrypted.
...
qmllint -I "$OMARCHY_PATH/shell" -I . Service.qml Settings.qml EntityRow.qml
```
...
repository; it lists every room and device in a home.
